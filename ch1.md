...
commands in the same order, you should obtain the same results.
...
Why am I explaining the notion of path in such a convoluted way? To make
you practice using the terms “path” and “graph” correctly.
...
phenomenon can be illustrated by performing the following two tests
(this time, the observations in $X$, $Y$, and $Z$ were generated by a
process of shape $X \rightarrow Y \leftarrow Z$, but I did not include
the code for this process since you were supposed to write it yourself
as a solution to exercise 5):
...
*seem* that achievements in sports are related to academic performance
if we have too many (or too few) college students in our sample, and it
may *seem* that attractive people are less nice if we study people on
dates.
